  ...
  return (
    <Formik initialValues={initialValues} validationSchema={validationSchema}  onSubmit={(values, { setErrors }) => {
      if (!values.aadhaar?.trim() && !values.epic?.trim() && !values.driving_licence?.trim()) {
        ...
        console.log("Form Submitted:", values); // ✅ Log values to console
        onSubmit(values, "");
      }
    }} >
...
/>
          ...
          {/* ✅ Submit Button */}
          <Button type="submit" variant="contained" color="primary" sx={{ marginTop: 5 }}  onClick={(e) => {
          e.preventDefault();
          onSubmit(values, "add_more"); // ✅ Pass "add_more"
        }}>
            {submitButtonText} 
          </Button>

          {addNext && (
  <Button type="button" variant="contained" color="secondary" sx={{ marginTop: 5, marginLeft: 2 }}
  onClick={(e) => {
    e.preventDefault();
    onSubmit(values, "save_next"); // ✅ Pass "save_next"
  }}
